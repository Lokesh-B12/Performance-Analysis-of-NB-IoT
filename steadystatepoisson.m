...
a = 0;
for k = 1:1:col1   
    tau11 = zeros(col,100001);
    tau12 = zeros(col,100001);
    tau13 = zeros(col,100001);
    tau22 = zeros(col,100001);
    tau23 = zeros(col,100001);
    tau33 = zeros(col,100001);
    ps11 = zeros(col,100001);
    ps12 = zeros(col,100001);
    ps13 = zeros(col,100001);
    ps22 = zeros(col,100001);
    ps23 = zeros(col,100001);
    ps33 = zeros(col,100001);
    thr1 = zeros(col,100001);
    thr2 = zeros(col,100001);
    thr3 = zeros(col,100001);
    pi100 = zeros(col,100001);
    pi200 = zeros(col,100001);
    pi300 = zeros(col,100001);
    ...
    for j = 1:1:col
       ...
       lambda1 = lam(k)*T(1);
        ...
        for i=2:1:100001
            ...
            p11 = 1-ps11(j,i);
            pi100(j,i) = 2*(1-p11)*(1-p12)*(1-p13)/(2*(1-p11)*(1-p12)*(1-p13)+lambda1*(1-p12)*(1-p13)*(2+(W(1)-1)*p11-(W(1)+1)*p11^R(1))+lambda1*p11^R(1)*(1-p11)*(1-p13)*(2+(W(2)-1)*p12-(W(2)+1)*p12^(R(2)-R(1)))+lambda1*p11^R(1)*p12^(R(2)-R(1))*(1-p12)*(1-p11)*(2+(W(3)-1)*p13-(W(3)+1)*p13^(R(3)-R(2))));
            tau11(j,i) = (1-p11^R(1))/(1-p11)*lambda1*pi100(j,i);
            tau12(j,i) = (1-p12^(R(2)-R(1)))/(1-p12)*lambda1*pi100(j,i)*p11^R(1);
            tau13(j,i) = (1-p13^(R(3)-R(2)))/(1-p13)*lambda1*pi100(j,i)*p11^R(1)*p12^(R(2)-R(1));
        ...
        p_drop1(k,j) = p11^R(1)*p12^(R(2)-R(1))*p13^(R(3)-R(2));
        p_drop2(k,j) = p22^(R(2))*p23^(R(3)-R(2));
        p_drop3(k,j) = p33^R(3);
        d1(k,j) = ((1+((W(1)-1)*p11/2 - (W(1)+1)*p11^R(1)/2))*T(1)/(1-p11) + (p11^R(1)*(1+((W(2)-1)*p12/2 - (W(2)+1)*p12^(R(2)-R(1))/2)))*T(2)/(1-p12) +(p11^R(1)*p12^(R(2)-R(1))*(1+((W(3)-1)*p13/2 - (W(3)+1)*p13^(R(3)-R(2))/2)))*T(3)/(1-p13)+p11^R(1)*T(1)+p11^R(1)*p12^(R(2)-R(1))*2*T(2))/1000;
        ...
        d_drop1(k,j) =  ((R(1)-1)*(W(1)+1)/2*T(1)+(R(2)-R(1)-1)*(W(2)+1)/2*T(2)+(R(3)-R(2)-1)*(W(3)+1)/2*T(3))/1000;
        ...
        throughput(k+a,j) = thr1(j,100001);
        throughput(k+a+1,j) = thr2(j,100001);
        throughput(k+a+2,j) = thr3(j,100001);
        psucc(k+a,j) = ps11(j,100001);
        psucc(k+a+1,j) = ps22(j,100001);
        psucc(k+a+2,j) = ps33(j,100001);
        pcoll(k+a,j) = p11;
        pcoll(k+a+1,j) = p22;
        pcoll(k+a+2,j) = p33;
    end
